...
Window::Window(uint32_t width, uint32_t height, const std::string& title,
  ...
  : mContext(std::make_unique<ContextImplementation>(
      width, height, title, rgbaBits[0], rgbaBits[1], rgbaBits[2], rgbaBits[3],
      depthBits, stencilBits, samples, resizable, true, true, nullptr,
      nullptr)),
    mRenderFunction(nullptr), mOnKeyFunction(nullptr),
    mOnMouseFunction(nullptr), mOnMouseMoveFunction(nullptr),
    mOnScrollFunction(nullptr), mOnResizeFunction(nullptr)
{
  mRenderFunction = [&]() { glClear(GL_COLOR_BUFFER_BIT); };
...
void Window::swapBuffers() const { glfwSwapBuffers(mContext->getGLFW()); }
